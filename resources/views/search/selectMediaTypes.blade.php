...
@section('content')
            ...
            <div class="col-xs-8">
                @if(isset($more))
                    <form method="POST" class="form-control-feedback"
                          action="{{ action('SearchController@selectRedirect', ['id_url' => $id_url]) }}">
    ...
    @else
        <div class="container">
            <div class="col-xs-8">
                ...
                @else
                    <form method="POST" class="form-control-feedback"
                          action="{{ action('SearchController@indexRedirect') }}">
                        <div class="form-group">
                            <label for="text"><h3>Search by ID <span
                                            class="defaultDatabase">{{ config('database.default') }}</span></h3></label>
                            <input type="text" class="input-group col-3" id="id" name="id">
    ...
    @if(isset($info))
        ...
        <table class="table table-hover mb-5">
            <tr align="center">
                <td>Watch in playster this {{ $mediaTypeTitle }} - <a
                            href="https://play.playster.com/{{ $mediaTypeTitle }}/{{ $info['id']}}/autumn-with-horses-trudy-nicholson"
