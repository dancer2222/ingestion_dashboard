...
<div class="mb-3 border-bottom">
    ...
    <span class="float-right">{{ $item->qaBatch->data_source_provider_id ?? '' }}
        [{{ $item->provider->get(0)->name ?? '' }}]</span>
</div>
        ...
        <span class="float-right">
            <form method="POST" class="form-group" id="form"
                ...
                <input type="hidden" id="object" name="object"
                       value="{{ \Ingestion\Search\Normalize::normalizeBucketName($item->licensor->name ?? '') }}/{{ str_replace(["Full_{$item->licensor->name}_", "Delta_{$item->licensor->name}_"], '', $item->qaBatch->title ?? '-') }}">
    ...
    <div class="mb-3 bottom">
        <span class="float-right">
            <form method="POST" class="form-group" id="form"
                ...
                <input type="hidden" id="id" name="id" value="{{ $item->id }}">
                <input type="hidden" name="_token" value="{{csrf_token()}}">
                <button type="submit" class="btn btn-secondary" id="aleks">Download AWS metadata file <b><i class="fab fa-aws"></i></b></button>
            </form>
        </span>
    </div>
</div>
